...
import {IInitialState} from "../../models/interfaces/redux/cart";
...
export const cartReducer = (state = initialState, {type, payload}: AnyAction): IInitialState => {
  switch (type) {
    case ADD_TO_CART:
      const {
        amount,
      ...
      if (temp) {
        ...
        return {...state, cart: [...state.cart, newProduct]} as IInitialState
      }
    ...
    case COUNT_CART_TOTALS:
      ...
      let totalItems: number = state.gift !== null ? 1 + totalItem : totalItem
      let totalAll: number = Number((totalAmount + SHIPPING_FEE).toFixed(2))
